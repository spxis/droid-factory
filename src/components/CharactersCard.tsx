...
function CharactersCard({ characters }: CharactersCardProps) {
  ...
  const sorted = useMemo(() => [...list].sort((left, right) => left.name.localeCompare(right.name)), [list]);
      ...
      <div className="flex flex-wrap gap-2">
        {sorted.map((character) => (
          <Link
            ...
            title={`${character.name}${character.species ? ` • ${character.species.name}` : ''}${character.homeworld ? ` • ${character.homeworld.name}` : ''}`}
